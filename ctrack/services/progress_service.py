"""Progress computation — actual vs budget vs expected spend for a period.

Extracted from the fat ``ProgressView`` so the view only parses query params,
calls :func:`resolve_period` / :func:`compute_progress`, and serializes the
result.
"""
...
def resolve_period(params, today):
    ...
    ``params`` is a mapping of query parameters (e.g. ``request.query_params``).
    An explicit ``from_date``/``to_date`` range takes priority over a named or
    numeric ``period``.
    """
...
def compute_progress(from_date, to_date, label, group_by, today=None):
    """Build the progress payload (``period``/``rows``/``totals``) for a period."""
    if today is None:
        today = date.today()
    ...
    if group_by == "category_group":
        spend_map = _spend_by_category_group(from_date, to_date)
    else:
        spend_map = _spend_by_category(from_date, to_date)
